---
title: "Análise de dados: Unimanual ND"
...
# Obter Meta dados dos arquivos Unimanual ND ------------------------------------------------
meta <- get_metaData('06UnimanualND')
...
Ao final do processo de detecção das fases do Unimanual ND, notou-se que, devido a heterogeinidade de formas do sinal, a rotina de deteção automática dos tempos limites não obteve sucesso em alguns casos, em especial para T4. Para solucionar o problema, realizou-se uma curadoria manual dos casos de falha.
...
#write_csv(picos,'../data/picosUND.csv')
tmp <- ajusta_XY(picos,
          picosFile = '../data/picosUND.csv',
...
#picos <- read_csv('../data/picosUND.csv')
...
Quando comparando os resultados, notamos que o grupo de **Controle não foi capaz de prever o comportamento das médias de M1 e M2**, extrapolando o *threshold* de 0.05 em ambos os casos. Porém, quando comparando M1 e M2, foi possível prever o comportamento da linhas de média de M2 com base em M1.

Assim, parece não haver uma real significância nos resultados observados para  intervenção em M1 e intervenção em M2, segundo o teste Granger.
...
Todos os testes indicaram um ρ > 0.64, confirmando assim a correlação entre os resultados obtidos pelo grupo de controle, M1 e M2.
...
# Análise das diferentes fases do Levantar+Unimanual ND

Para a análise das diferentes fases do levantar+Unimanual ND, foram utilizados os seguintes parâmetros : ÁreaCoP, Velocidade ântero-posterior, Velocidade médio-lateral, Amplitude de deslocamento do CoP ântero-posterior e Amplitude de deslocamento do CoP médio lateral (ML).
...
### Parâmetros analisados por fase do levantar+Unimanual ND
