---
title: "Análise de dados: Unimanual D"
...
O estudo toma como dados a leitura de uma plataforma de força sobre a qual o paciente levanta-se . Foram coletados dados de um grupo que sofreu a Intervenção e outro de Controle, em dois momentos  distintos: Pré-Intervenção (M1) e Pós-Intervenção (M2).

Foram disponibilizados os arquivos de dados dos seguintes pacientes, com resultados em triplicata:

```{r}

# Obter Meta dados dos arquivos Unimanual D ------------------------------------------------
meta <- get_metaData('05UnimanualD')
...
Também foi calculado o tempo médio de todas as  leituras efetuadas. Foram considerados picos máximos de T2 somente valores abaixo deste valor médio. Isso foi necessário, porque algumas amostras apresentaram valores de leitura finais superiores ao pico T2.
...
itvM2$fzN<-0
```
# Encontrar o T2 do gráfico dos pacientes

O momento em que o paciente levanta-se do assento é caracterizado por um pico no gráfico de força. Aqui foi realizada a detecção deste pico. 

Também foram estimados os fatores de correção dos valores de força zero, assim como foi calculado o valor base para translação dos gráficos sobre o eixo do tempo, de forma que todos os picos T2 pudessem ser plotados com um mesmo valor para o eixo X.
...
Ao final do processo de detecção das fases do Unimanual D, notou-se que, devido a heterogeinidade de formas do sinal, a rotina de deteção automática dos tempos limites não obteve sucesso em alguns casos, em especial para T4. Para solucionar o problema, realizou-se uma curadoria manual dos casos de falha.

Tempos antes da curadoria:

```{r}
print(picos)
#intervenção manual no ajuste dos Ts
#write_csv(picos,'../data/picosUD.csv')
tmp <- ajusta_XY(picos,
          picosFile = '../data/picosUD.csv',
...
Como as amplitudes obtidas pela plataforma são dependentes do peso do paciente e da força por ele aplicada para levantar-se, optou-se por realizar uma normalização nos valores de intensidade de sinal obtido, a fim de facilitar a comparação visual dos resultados dos diversos pacientes. 
...
```{r, ctlM1M2, fig.cap="\\label{fig:ctlM1M2}Dados do grupo de controle em M1 e M2. A linha azul indica o tempo M1 e a vermelha M2. As linhas verticais ciano e magenta identificam os pontos T1, T2, T3 e T4 dos tempos M1 e M2, respectivamente."}
...
for(ind in unique(ctlM1$id)){
  ...
  gCtl[[idxC]] <- 
    ...
    geom_line(data = ctlM2[ctlM2$id == ind,],
              aes(x=tempo, y= fzS),
              color='red')+
  geom_vline(xintercept = T11, color='cyan4', linetype = "dotdash", size= 0.3)+
  geom_vline(xintercept = T21, color='cyan4', linetype = "dotdash", size= 0.3)+
  geom_vline(xintercept = T31, color='cyan4', linetype = "dotdash", size= 0.3)+
  geom_vline(xintercept = T41, color='cyan4', linetype = "dotdash", size= 0.3)+
  geom_vline(xintercept = T12, color='magenta', linetype = "dotdash", size= 0.3)+
  geom_vline(xintercept = T22, color='magenta', linetype = "dotdash", size= 0.3)+
  geom_vline(xintercept = T32, color='magenta', linetype = "dotdash", size= 0.3)+
  geom_vline(xintercept = T42, color='magenta', linetype = "dotdash", size= 0.3)+
  annotate('text',label = c('T1','T2','T3','T4'), 
           x=c((T11+T12)/2,(T21+T22)/2,(T31+T32)/2,(T41+T42)/2),
           y=c(rep(min,4)),
           color='magenta')+
...
```{r, itvM1M2, fig.cap="\\label{fig:itvM1M2}Dados do grupo de intervenção em M1 e M2. A linha azul indica o tempo M1 e a vermelha M2. As linhas verticais ciano e magenta identificam os pontos T1, T2, T3 e T4 dos tempos M1 e M2, respectivamente."}
#picos <- read_csv('../data/picosUD.csv')
...
ggplot()+
  ...
  geom_vline(xintercept = T1, color='magenta', linetype = "dotdash", size= 0.3)+
  geom_vline(xintercept = T2, color='magenta', linetype = "dotdash", size= 0.3)+
  geom_vline(xintercept = T3, color='magenta', linetype = "dotdash", size= 0.3)+
  geom_vline(xintercept = T4, color='magenta', linetype = "dotdash", size= 0.3)+
  annotate('text',label = c('T1','T2','T3','T4'), 
           x=c(T1+0.25,T2+0.25,T3+0.25,T4+0.25),
           y=c(-1,-1,-1,-1),
           color='magenta')
```
...
É importante lembrar que as amplitudes, embora bastante heterogêneas, para esta análise possuem importância apenas para definir as diferentes fases do processo de levantar, já que estão intimamente ligadas ao peso do paciente.
...
Quando comparando os resultados, notamos que o p-valor da comparação Controle x M2 extrapolou o *threshold* ligeiramente, alcançando 0.06807. Porém, nos demais cenários houve significância nos resultados, o que indica que devemos descartar a hipótese nula. Ou seja, de posse da linha média dos controles é possível prever o comportamento da linhas média de M1, mas não de M2. Porém com base em M1 é possível prever M2.

Assim, parece não haver uma real significância nos resultados observados para  controle, intervenção em M1 e intervenção em M2, segundo o teste Granger.

## Teste de Correlação de Spearman

Este é um teste não paramétrico que verifica a dependência entre postos de variáveis. Assim ele analisa se existe uma covariância entre os valores de duas variáveis (v1 e v2), ou, em outras palavras, se os valores de dias variáveis aumentam ou diminuem simultaneamente.
...
ggplot(data = data)+
  ...
  # geom_line(aes(t,d2),color='blue')+
  # geom_line(aes(t,d3),color='green')+
  geom_vline(xintercept = T1, color='magenta', linetype = "dotdash", size= 0.3)+
  geom_vline(xintercept = T2, color='magenta', linetype = "dotdash", size= 0.3)+
  geom_vline(xintercept = T3, color='magenta', linetype = "dotdash", size= 0.3)+
  geom_vline(xintercept = T4, color='magenta', linetype = "dotdash", size= 0.3)+
...
Todos os testes indicaram um ρ > 0.82, confirmando assim a correlação entre os resultados obtidos pelo grupo de controle, M1 e M2.
...
# Análise das diferentes fases do Levantar+Unimanual D

Para a análise das diferentes fases do levantar+Unimanual D, foram utilizados os seguintes parâmetros : ÁreaCoP, Velocidade ântero-posterior, Velocidade médio-lateral, Amplitude de deslocamento do CoP ântero-posterior e Amplitude de deslocamento do CoP médio lateral (ML).
...
ggplot(metricFase,aes(x = trat, y = valor, col = group))+
  #geom_boxplot()+
  ggtitle('Comparativo de parâmetros entre fases')+
...
### Parâmetros analisados por fase do levantar+Unimanual D
...
for(momento in 1:2) {
  for(trat in param){
    for( fase in 1:3){
      tmp <- metricFase[metricFase$moment == momento &
                       metricFase$trat == trat &
                         metricFase$fase == fase,
                     c('group','valor')]
      cat('##################################\n\n',
          paste0('M',momento),' Parâmetro:', trat,
          'Fase:', fase,'\n')
...
# ks.test(d2,d3)
# 
# cor.test(itv1$fzMN, ctl$fzMN,
#          method = "spearman",
#          continuity = FALSE,
#          conf.level = 0.99)
# ks.test(itv2$fzMN, ctl$fzMN)
# ks.test(itv1$fzMN, itv2$fzMN)
# # 
# # ks.test(itvM1$fzN[itvM1$id == 'Ind12'], ctl$fzMN,alternative = 'l')


# ind='Ind28'
# for(ind in unique(df_f$id[df_f$grupo == 'Controle'&
#                 df_f$moment == '1'])){
#   print(ggplot(data = df_f[df_f$id == ind &
#                        df_f$grupo == 'Controle'&
#                        df_f$moment == '1',
#                      c('tempo','fz')])+
#     ggtitle(ind)+
#     geom_line(aes(x=tempo, y= fz,)))
#   g
# }
# 
# 
# pspectrum(df_f$fz[df_f$id=='Ind3' & 
#                             df_f$rep == '3' ],
#           x.frqsamp =1000, plot = T)
# 
# 
# save(df_f, file = "dff.RData")
#   df_a <- calculate_area_cop(df_f)
#   
#   res <- data.frame(t(df_a), id = meta_m1$ind[.x], trat = meta_m1$trat[.x])
#   res
#   
# }) -> m3
# 
